async function GetMonthlySummary(req, res, next) {
    ...
    try {
        ...
        let summary = await Promise.all(users.map(async (user) => {
            ...
            let totalSystolic = measurements.reduce((acc, m) => acc + m.systolic, 0);
            let totalDiastolic = measurements.reduce((acc, m) => acc + m.diastolic, 0);
            ...
            let abnormalCount = measurements.filter(m => m.systolic > avgSystolic * 1.2).length;
            return {
                user: user.name,
                avgSystolic,
                avgDiastolic,
